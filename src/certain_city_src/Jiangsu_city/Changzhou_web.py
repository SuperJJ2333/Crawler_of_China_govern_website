...
def extract_news_info(json_data, year, page_num, logger, base_url=None):
    ...
    if 'result' in data:
        ...
        for news_item in data_list:
            ...
            try:
                # 提取发布日期
                date = news_item['date']['raw']
                # 提取新闻标题
                title = news_item['title']['raw']
                # 提取新闻URL
                news_url = news_item['url']['raw']
...
if __name__ == '__main__':
    ...
    # 爬虫基本信息
    city_info = {'name': 'Changzhou',
                 'province': 'Jiangsu',
                 'total_news_num': 10000,
                 'each_page_num': 20,
                 'targeted_year': 2019,
                 'base_url': 'https://search.jscz.org.cn:7443/manda-app/api/app/search/v1/1f59ixu/search'
                 }
    # 可选参数
    thread_num = 5

    """post信息"""
    # data_dict = '{"cid":"cncq4mFH6YqfUxGe2sJ6j1QJiHGNNLsl","uid":"cncq4mFH6YqfUxGe2sJ6j1QJiHGNNLsl","query":"学习考察 ' \
    #             '考察学习","current":"4","size":20,"disable_correction":false,"facets":{"view":[{"type":"value",' \
    #             '"name":"view","sort":{"count":"desc"},"size":10}]},"input_type":"Input"}'
    # # JSON字符串，通常从API或其他源接收
    json_string = '{"cid":"cncq4mFH6YqfUxGe2sJ6j1QJiHGNNLsl","uid":"cncq4mFH6YqfUxGe2sJ6j1QJiHGNNLsl","query":"学习考察 考察学习","current":"4","size":20,"disable_correction":false,"facets":{"view":[{"type":"value","name":"view","sort":{"count":"desc"},"size":10}]},"input_type":"Input"}'

    # 将JSON字符串转换成字典
    data_dict = json.loads(json_string)

    page_num_name = "current"
    # off_set = 10
    ...
    headers = {
        ...
        "Host": "search.jscz.org.cn:7443",
        "Origin": "https://www.changzhou.gov.cn",
        "Referer": "https://www.changzhou.gov.cn/",
